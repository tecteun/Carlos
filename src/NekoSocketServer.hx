...
class NekoSocketServer extends ThreadServer<Client, Message>
{
	...
	override function clientConnected( s : Socket ) : Client
	{
		...
		#end
		return { socket: s, id: id, state: s.custom != null ? STATE.forwarding : STATE.handshake, buffer: new BytesBuffer(), socksocket: s.custom };
	}
	...
	override function readClientMessage(c:Client, buffer:Bytes, pos:Int, len:Int)
	{
		...
		switch(c.state) {
			...
			case STATE.forwarding	:	trace("fwd" + buffer.toString());  retval =  { str: buffer.toString(), bytes:buffer };  c.socksocket.output.write(buffer.sub(pos, len)); c.socksocket.output.flush();
	...
	private function handleRequest(c:Client):Message {
		...
		if(addressType == 0x01) { // ipv4
			...
			expandAndCopy(c, buffer.sub(10, buffer.length - 10));
		}
			...
			expandAndCopy(c, buffer.sub(responseBuf.length, buffer.length - responseBuf.length ));
		...
		var sock = new Socket();
		sock.custom = c.socket;
		sock.connect(new Host(host), port);
		addSocket(sock);
		c.socksocket = sock;
		c.state = STATE.forwarding;
		if(null != responseBuf){
			responseBuf.set(1, 0);
			responseBuf.set(2, 0);
			c.socket.output.write(responseBuf);
		}
		c.socket.output.flush();
